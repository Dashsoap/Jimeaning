...
const NOVEL_REWRITE_RULES = `## 小说洗稿规则
...
const ANTI_AI_RULES = `## 去AI化规则（最高优先级）
...
export const REWRITE_SCRIPT_SYSTEM = REWRITE_SYSTEM;
export const REWRITE_SCRIPT_USER = REWRITE_USER;
export const REWRITE_SCRIPT_CHUNK_USER = (
  content: string,
  prompt: string,
  chunkIndex: number,
  totalChunks: number,
) => REWRITE_CHUNK_USER(content, prompt, chunkIndex, totalChunks);
