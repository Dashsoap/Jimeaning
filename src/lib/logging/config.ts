...
function parseLevel(value?: string | null): LogLevel {
  const upper = (value || "INFO").trim().toUpperCase();
...
export const LOG_CONFIG = {
  ...
  service: (process.env.LOG_SERVICE || "jimeaning").trim(),
  redactKeys: (
    process.env.LOG_REDACT_KEYS ||
    "password,token,apiKey,apikey,authorization,cookie,secret,access_token,refresh_token"
  )
    .split(",")
