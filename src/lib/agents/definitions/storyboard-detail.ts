/**
 * Agent: storyboard-detail — 分镜细化 + video_prompt 生成 (Phase 3)
 *
 * Takes Phase 1 storyboard plan and generates:
 * - shot_type (景别+视角)
 * - camera_move (镜头运动)
 * - video_prompt (动态视频提示词，用年龄性别替代角色名)
 *
 * Adapted from waoowaoo's agent_storyboard_detail.zh.txt
 */

import type { AgentDef } from "../types";
import type { StoryboardResult, StoryboardShot } from "./storyboard-director";

export interface StoryboardDetailInput {
...
/** Build a flat shot summary for the LLM from nested scenes structure */
function flattenShots(storyboard: StoryboardResult): Array<Record<string, unknown>> {
...
/** Guess age bracket from appearance string */
function buildCharacterAgeGender(characters: Array<{ name: string; appearance: string }>): string {
  return characters
    .map((c) => `${c.name}：${c.appearance}`)
    .join("\n");
}

export const storyboardDetailAgent: AgentDef<StoryboardDetailInput, StoryboardDetailResult> = {
  name: "storyboard-detail",
  description: "为分镜生成动态 video_prompt（Phase 3）",
  outputMode: "json",
  temperature: 0.4,

  systemPrompt: () => `你是顶级电影分镜师。根据分镜规划和场景类型，设计镜头语言和视频提示词。
...
- 情感场景："年轻女子缓缓低下头，泪珠沿脸颊滑落，抬手抹去眼角，镜头轻轻环绕她"
...
9. ⚠️ JSON安全：所有引号（""''等）在 JSON 字符串值中必须统一替换为「」，严禁出现未转义的英文双引号 "`,
  ...
  userPrompt: (input) => {
    const shots = flattenShots(input.storyboard);
    const charAgeGender = buildCharacterAgeGender(input.characters);

    return `为以下 ${shots.length} 个分镜生成 video_prompt：
