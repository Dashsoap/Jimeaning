...
export interface ReviewInput {
  ...
  /** Source content type from style fingerprint — used to select correct review criteria for "same" format */
  contentType?: string;
}
...
export interface ReviewResult {
  ...
  dimensions: {
    ...
    hookAndPaywall?: { score: number; notes: string };
...
### 6. 钩子与付费点（hookAndPaywall）— 满分10
| 分数 | 标准 |
|------|------|
| 9-10 | 钩子密集，每集结尾让人必须看下集 |
| 7-8 | 钩子基本到位，吸引力一般 |
| 5-6 | 钩子偏弱，吸引力不够 |
| 3-4 | 缺乏有效钩子 |

### 7. 角色一致性（characterConsistency）— 满分10
| 分数 | 标准 |
|------|------|
| 9-10 | 角色行为/语气/决策完全符合人设，跨章节一致 |
| 7-8 | 主要角色一致，偶有细微偏差 |
| 5-6 | 角色行为有时不符合人设 |
| 3-4 | 角色行为与人设矛盾 |

## 门控逻辑
- 通过（≥49分）：可进入分镜阶段（7维×10分=70分，70%=49）
- 不通过（<49分）：列出具体问题和修改建议
...
| 分数 | 标准 |
|------|------|
| 9-10 | 钩子密集，第3-5集有强付费悬念，每集结尾让人必须看下集 |
| 7-8 | 钩子基本到位，付费点有但不够强烈 |
| 5-6 | 钩子偏弱，付费点位置不对或吸引力不够 |
| 3-4 | 缺乏有效钩子 |
...
## 门控逻辑
- 通过（≥49分）：改写质量合格（7维×10分=70分，70%=49）
- 不通过（<49分）：列出具体问题和修改建议
...
dimensions 字段使用以下key：faithfulness, proseQuality, pacing, humanness, styleConsistency, hookAndPaywall, characterConsistency
...
export const reviewDirectorAgent: AgentDef<ReviewInput, ReviewResult> = {
  ...
  systemPrompt: (input) => {
    ...
    if (fmt === "same") {
      // Use contentType from style fingerprint to select correct review criteria
      const ct = input.contentType?.toLowerCase() ?? "";
      if (ct.includes("script") || ct.includes("screenplay")) {
