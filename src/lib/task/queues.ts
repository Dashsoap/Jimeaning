...
export function getQueueByType(type: string): Queue {
  if (type.includes("IMAGE") || type === "PANEL_VARIANT" || type === "IMAGE_CHARACTER" || type === "IMAGE_LOCATION") return getImageQueue();
...
/**
 * Attempt to remove a BullMQ job by its ID across all queues.
  ...
  for (const queue of getAllQueues()) {
    ...
    if (job) {
      try {
        ...
        // Job may already be active/completed — that's fine
      }
      return true;
    }
  }
